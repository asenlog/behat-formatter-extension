<?php
/**
 * Created by PhpStorm.
 * User: Angelos Roussakis (aggelos.roussakis@gmail.com)
 * Date: 06/04/18
 * Time: 12:50
 */

namespace App\Printer;

use Behat\Testwork\Output\Exception\BadOutputPathException;
use Behat\Testwork\Output\Printer\OutputPrinter as PrinterInterface;
use App\Renderer\BaseRenderer;

class FileOutputPrinter implements PrinterInterface
{
    const XML_EXTENSION = 'xml';
    const HTML_EXTENSION = 'html';

    /**
     * @param  $outputPath string where to save the generated report file
     */
    private $outputPath;

    /**
     * @param  $base_path string Behat base path
     */
    private $base_path;

    /**
     * @param  $rendererFiles array List of the filenames for the renderers
     */
    private $rendererFiles;

    /**
     * @var array
     */
    private $extensions;

    /**
     * Generating filenames for the html files
     *
     * @param $rendererList
     * @param $filenames
     * @param $base_path
     */
    public function __construct($rendererList, $filenames, $base_path)
    {
        $filenames = explode(",", $filenames);
        $this->rendererFiles = array();
        foreach ($rendererList as $index => $renderer) {
            if ($filenames == 'generated') {
                $date = date('YmdHis');
                $this->rendererFiles[$renderer] = $renderer . '_' . $date;
            } else {
                if ($renderer == BaseRenderer::TESTRAIL_RENDERER) {
                    $this->extensions[$renderer] = self::XML_EXTENSION;
                } else {
                    $this->extensions[$renderer] = self::HTML_EXTENSION;
                }
                $this->rendererFiles[$renderer] = $filenames[$index];
            }
        }

        $this->base_path = $base_path;
    }

    /**
     * Verify that the specified output path exists or can be created,
     * then sets the output path.
     *
     * @param String $path Output path relative to %paths.base%
     *
     */
    public function setOutputPath($path)
    {
        $outputPath = $path;
        if (!file_exists($outputPath)) {
            if (!mkdir($outputPath, 0755, true)) {
                throw new BadOutputPathException(
                    sprintf(
                        'Output path %s does not exist and could not be created!',
                        $outputPath
                    ),
                    $outputPath
                );
            }
        } else {
            if (!is_dir(realpath($outputPath))) {
                throw new BadOutputPathException(
                    sprintf(
                        'The argument to `output` is expected to the a directory, but got %s!',
                        $outputPath
                    ),
                    $outputPath
                );
            }
        }
        $this->outputPath = $outputPath;
    }

    /**
     * Returns output path
     *
     * @return String output path
     */
    public function getOutputPath()
    {
        return $this->outputPath;
    }

    /**
     * Sets output styles.
     *
     * @param array $styles
     */
    public function setOutputStyles(array $styles)
    {
    }

    /**
     * Returns output styles.
     *
     * @return array
     */
    public function getOutputStyles()
    {
    }

    /**
     * Forces output to be decorated.
     *
     * @param Boolean $decorated
     */
    public function setOutputDecorated($decorated)
    {
    }

    /**
     * Returns output decoration status.
     *
     * @return null|Boolean
     */
    public function isOutputDecorated()
    {
        return true;
    }

    /**
     * Sets output verbosity level.
     *
     * @param integer $level
     */
    public function setOutputVerbosity($level)
    {
    }

    /**
     * Returns output verbosity level.
     *
     * @return integer
     */
    public function getOutputVerbosity()
    {
    }

    /**
     * Writes message(s) to output console.
     *
     * @param string|array $messages message or array of messages
     */
    public function write($messages = array())
    {
        //Write it for each message = each renderer
        foreach ($messages as $key => $message) {
            $dir = $this->outputPath . DIRECTORY_SEPARATOR . $this->extensions[$key] . DIRECTORY_SEPARATOR;
            if (!file_exists($dir)) {
                mkdir($dir, 0775, true);
            }
            $file = $dir . $this->rendererFiles[$key] . '.' . $this->extensions[$key];
            file_put_contents($file, $message);
            $this->copyAssets($key);
        }
    }


    /**
     * Writes newlined message(s) to output console.
     *
     * @param string|array $messages message or array of messages
     */
    public function writeln($messages = array())
    {
        //Write it for each message = each renderer
        foreach ($messages as $key => $message) {
            $dir = $this->outputPath . DIRECTORY_SEPARATOR . $this->extensions[$key] . DIRECTORY_SEPARATOR;
            if (!file_exists($dir)) {
                mkdir($dir, 0775, true);
            }
            $file = $dir . $this->rendererFiles[$key] . '.' . $this->extensions[$key];
            file_put_contents($file, $message, FILE_APPEND);
        }
    }

    /**
     * Writes  message(s) at start of the output console.
     *
     * @param string|array $messages message or array of messages
     */
    public function writeBeginning($messages = array())
    {
        //Write it for each message = each renderer
        foreach ($messages as $key => $message) {
            $dir = $this->outputPath . DIRECTORY_SEPARATOR . $this->extensions[$key] . DIRECTORY_SEPARATOR;
            if (!file_exists($dir)) {
                mkdir($dir, 0775, true);
            }
            $file = $dir . $this->rendererFiles[$key] . '.' . $this->extensions[$key];
            $fileContents = file_get_contents($file);
            file_put_contents($file, $message . $fileContents);
        }
    }

    /**
     * Copies the assets folder to the report destination.
     *
     * @param string : the renderer
     */
    public function copyAssets($renderer)
    {
        // If the assets folder doesn't exist in the output path for this renderer, copy it
        $source = realpath(dirname(__FILE__));
        $assets_source = realpath($source . '/../../assets/' . $renderer);
        if ($assets_source === false) {
            //There is no assets to copy for this renderer
            return;
        }

        //first create the assets dir
        $destination = $this->outputPath . DIRECTORY_SEPARATOR
            . $this->extensions[$renderer] . DIRECTORY_SEPARATOR . 'assets';
        @mkdir($destination);

        $this->recurseCopy($assets_source, $destination . DIRECTORY_SEPARATOR . $renderer);
    }

    /**
     * Recursively copy a path.
     * @param $src
     * @param $dst
     */
    private function recurseCopy($src, $dst)
    {
        $dir = opendir($src);
        @mkdir($dst);
        while (false !== ($file = readdir($dir))) {
            if (($file != '.') && ($file != '..')) {
                if (is_dir($src . '/' . $file)) {
                    $this->recurseCopy($src . '/' . $file, $dst . '/' . $file);
                } else {
                    copy($src . '/' . $file, $dst . '/' . $file);
                }
            }
        }
        closedir($dir);
    }

    /**
     * Clear output console, so on next write formatter will need to init (create) it again.
     */
    public function flush()
    {
    }
}
